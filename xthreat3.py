...
with open("xThreat_data.json", "r") as fd:
   xthreat_data = json.load(fd)
...
for filename in os.listdir("2023_worlds_replays"):
  ...
  while i < len(d["gameStats"]["hits"]):
     ...
     if not hit["isKickoff"]:
        ...
        zone_hit = find_zone_str(xthreat_data, (pos_x, pos_y))
        ...
        if i + 1 == len(d["gameStats"]["hits"]):
           ...
           next_zone_hit = find_zone_str(xthreat_data, get_ball_position((next_hit["ballData"]["posX"], next_hit["ballData"]["posY"]), find_team_colour(find_team(hit["playerId"]["id"], teams), d)))
        ...
        if last_hit:
           ...
           if k not in player_data[player_hit]["games"]:
              player_data[player_hit]["games"].append(k)
           ...
           if is_goal(d, i, teams):
              ...
              #total_goals += 1
              player_data[player_hit]["goals"] += 1
              player_data[player_hit]["xthreat_dif"].append(1 - xthreat_data[zone_hit]["xThreat"])
              i += 1
              continue
           ...
           if next_player_hit == player_hit or on_same_team(next_player_hit, player_hit, teams):
              ...
              player_data[player_hit]["xthreat_dif"].append(xthreat_data[zone_hit]["xThreat"] - xthreat_data[next_zone_hit]["xThreat"])
              player_data[player_hit]["xthreat_dif_excl_goals"].append(xthreat_data[next_zone_hit]["xThreat"] - xthreat_data[zone_hit]["xThreat"])
